 ...
 */
function rejectionFromParams(params: URLSearchParams): AuthRejection | null {
  ...
  const meta: AuthRejectionMeta = {}
  if (allowedDomains?.length) meta.allowedDomains = allowedDomains
  if (presentedDomain) meta.presentedDomain = presentedDomain
  if (emailVerified !== undefined) meta.emailVerified = emailVerified
...
export class RealAuthCore extends BaseCore {
  ...
  private static readonly LOAD_BACKOFF_MS = [400, 800, 1500, 2500, 4000, 5000]
    ...
    if (!this.snapshot.isSignedIn) {
      return {
        error: {
          ...
          longMessage:
            "The sign-in handshake returned, but no active session was established. The attempt " +
  ...
  private scheduleRefresh(): void {
    this.cancelRefreshTimer()
    if (!this.autoRefresh || !this.activeSessionId) return
    const now = Math.floor(Date.now() / 1000)
    ...
    this.refreshTimer = setTimeout(() => {
      ...
      if (this.activeSessionId) void this.refresh().catch(() => null)
    }, secs * 1000)
  }
